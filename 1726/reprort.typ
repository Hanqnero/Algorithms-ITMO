...
Город Екатеринбург представляет собой прямоугольник со сторонами, ориентированными по сторонам света. Все улицы города идут строго с запада на восток или с севера на юг, проходя через весь город от края до края. Дома всех членов программного комитета расположены строго на пересечении каких-то двух перпендикулярных улиц. Известно, что все члены комитета ходят только по улицам, поскольку идти по тротуару гораздо приятнее, чем по дворовым тропинкам. И, конечно, при переходе от дома к дому они всегда выбирают кратчайший путь (поскольку улицы идут строго горизонтально и вертикально, это означает, что расстояние между домами с координатами (x1, y1) и (x2, y2) равно |x1 - x2| + |y1 - y2|). Программный комитет очень дружный, и все его члены ходят в гости ко всем одинаково часто.

== Исходные данные

Первая строка содержит целое число `n` — количество членов программного комитета (2 ≤ `n` ≤ 10<sup>5</sup>).
В `i`-й из следующих `n` строк через пробел записаны целые числа `xi`, `yi` — координаты дома `i`-го члена программного комитета (1 ≤ `xi`, `yi` ≤ 10<sup>6</sup>).
...
_Автор задачи: Денис Дублённых_ \
_Источник задачи: Уральская региональная командная олимпиада по программированию 2009_
